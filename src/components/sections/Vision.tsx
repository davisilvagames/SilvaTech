...
export default function Vision() {
  ...
  return (
    <Section id="visao" className="py-[100px] sm:py-[140px]">
        ...
        <div ref={ref} className={`transition-all duration-700 ${
          ...
          <div className="grid sm:grid-cols-3 gap-6 sm:gap-8">
            {[
              ...
              <div
                key={item.title}
                className={`rounded-xl border border-border bg-bg-card p-7 text-left transition-all duration-500 hover:border-accent/20 ${
                  isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                }`}
                style={{ transitionDelay: `${200 + i * 100}ms` }}
              >
                <h3 className="text-text-primary font-semibold mb-3">{item.title}</h3>
