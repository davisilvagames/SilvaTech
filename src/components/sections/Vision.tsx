...
export default function Vision() {
  ...
  return (
    <Section id="visao" className="py-[120px] sm:py-[180px]">
      <div className="max-w-[800px] mx-auto px-6 text-center">
        <div ref={ref} className={`transition-all duration-700 ${
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-text-primary leading-tight mb-10">
            Estamos apenas começando.
          </h2>
          <p className="text-text-secondary text-lg max-w-2xl mx-auto leading-relaxed mb-14">
          ...
          <div className="grid sm:grid-cols-3 gap-8">
            {[
              ...
              <div
                key={item.title}
                className={`rounded-xl border border-border bg-bg-card p-8 text-left transition-all duration-500 hover:border-accent/20 ${
                  isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                }`}
                style={{ transitionDelay: `${200 + i * 100}ms` }}
              >
                <h3 className="text-text-primary font-semibold mb-4">{item.title}</h3>
